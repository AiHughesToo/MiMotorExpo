...
import { registerUser, emailChanged, passwordChanged, nameChanged, vinChanged, riderRequirements } from '../actions';
...
class SignUp extends Component {
  ...
  onVinChange(text) {
    ...
    if(text.length < 10 && text.length > 5){
      this.props.riderRequirements(true);
    } else {
      this.props.riderRequirements(false);
    }
  }
    ...
    return(
      <Card>
        ...
        <CardSection>
          <Input
            label={'Motor Vin #'}
            placeholder={'Motor Vin Number'}
            onChangeText={this.onVinChange.bind(this)}
            value={this.props.vinNumber}
          />
        </CardSection>
        ...
        <CardSection>
          <Input
            label={'Motor Plack #'}
            placeholder={'Motor Plack #'}
          />
        </CardSection>

          {this.renderError()}
        <CardSection>
          {this.renderRiderButton(this.props.riderRequirementsMet)}
        </CardSection>
      </Card>
    );
  }

  renderRiderButton(riderRequirements) {
  
    if (this.props.loading) {
      return <Spinner />;
    }

    if (riderRequirements == true) {
      return(
      ...
      <ImageBackground source={require('../../assets/main_background.png')} style={backgroundImage}>
        <KeyboardAwareScrollView
          ...
          extraHeight={Platform.select({ android: 250 })}>
            ...
            <SelectAccountBar />
            <Text style={sectionTitleTextStyle}> {this.props.accountType} </ Text>
            {this.renderForm()}
            </View>
        </KeyboardAwareScrollView>
      </ ImageBackground>
  ...
  const mapStateToProps = state => {
   return {
    ...
    vinNumber: state.auth.vinNumber,
    riderRequirementsMet: state.auth.riderRequirements,
    };
  };

export default connect(mapStateToProps, { emailChanged, passwordChanged, registerUser, nameChanged, vinChanged, riderRequirements }) (SignUp);
